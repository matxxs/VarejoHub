...
public class AuthenticationService : IAuthenticationService
{
    ...
    private async Task SendLoginMagicLink(string email, string token, User user, Supermarket? supermarket)
    {
        ...
        var magicLinkUrl = $"http://localhost:5197/api/auth/magic-login?token={token}&email={email}";
    ...
    {
        var magicLinkUrl = $"http://localhost:5197/api/auth/confirm-registration?token={token}&email={email}";
