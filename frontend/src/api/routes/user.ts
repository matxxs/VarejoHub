...
import { Supermarket } from "../routes/auth";
...
export async function getUsersBySupermarket(supermarketId: number): Promise<Result<User[]>> {
    try {
        const response = await privateApi.get<User[]>(`/user/supermarket/${supermarketId}/users`)
        return {
            isSuccess: true,
            value: response.data,
            error: "",
...
export async function getUserById(productId: number): Promise<Result<User>> { 
    try {
        const response = await privateApi.get<User>(`/user /${productId}`)
