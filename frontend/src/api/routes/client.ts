...
export async function getClientsBySupermarket(supermarketId: number): Promise<Result<Client[]>> {
    try {
        const response = await privateApi.get<Client[]>(`/client/supermarket/${supermarketId}`);
        return {
            isSuccess: true,
            value: response.data,
            error: "",
...
export async function getClientById(clientId: number): Promise<Result<Client>> {
    try {
        const response = await privateApi.get<Client>(`/client/${clientId}`);
        return {
            isSuccess: true,
            value: response.data,
            error: "",
...
export async function createClient(client: Omit<Client, 'idCliente'>): Promise<Result<Client>> {
    try {
        const response = await privateApi.post<Client>('/client', client);
        return {
            isSuccess: true,
            value: response.data,
            error: "",
...
export async function updateClient(clientId: number, client: Client): Promise<Result<void>> {
    try {
        await privateApi.put(`/client/${clientId}`, client);
        return {
            isSuccess: true,
            error: "",
        };
...
export async function deleteClient(clientId: number): Promise<Result<void>> {
    try {
        await privateApi.delete(`/client/${clientId}`);
        return {
            isSuccess: true,
            error: "",
        };
...
export async function searchClients(supermarketId: number, name: string): Promise<Result<Client[]>> {
    try {
        const response = await privateApi.get<Client[]>(`/client/supermarket/${supermarketId}/search`, {
            params: { name }
        });
        return {
            isSuccess: true,
